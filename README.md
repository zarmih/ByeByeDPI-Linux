...
* **Локальная история прогонов**: Ваши последние 20 запусков тестов сохраняются автоматически. Нажмите кнопку **History** в окне библиотеки, чтобы открыть историю.
* **Приватность**: История хранится **исключительно локально** в стандартном каталоге пользовательских данных вашей ОС (через `QStandardPaths.AppDataLocation`) и никуда не отправляется.
